...
export default function ImageGallery({ images }: ImageGalleryProps) {
  ...
  return (
    <div className="lg:-ml-12 lg:sticky lg:top-32 lg:col-start-2 lg:row-span-2 lg:row-start-1">
      ...
      <div className="relative aspect-video w-full max-w-[48rem] sm:max-w-[57rem] rounded-xl ring-1 ring-slate-200 dark:ring-slate-600 bg-slate-50 dark:bg-slate-800 group">
        <Image
          alt={currentImage.alt}
          src={currentImage.src}
          width={currentImage.width}
          height={currentImage.height}
          className="h-full w-full object-contain bg-transparent rounded-xl"
          priority={true}
          placeholder="blur"
        ...
        {images.length > 1 && (
          <>
            ...
            <button
              ...
              className="absolute left-0 top-0 bottom-0 w-1/3 flex items-center justify-start pl-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-gradient-to-r hover:from-black/10 hover:to-transparent dark:hover:from-white/10 dark:hover:to-transparent rounded-l-xl"
              aria-label="Previous image"
            >
              <div className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm rounded-full p-2 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110">
                <svg
            ...
            <button
              onClick={goToNext}
              className="absolute right-0 top-0 bottom-0 w-1/3 flex items-center justify-end pr-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-gradient-to-l hover:from-black/10 hover:to-transparent dark:hover:from-white/10 dark:hover:to-transparent rounded-r-xl"
      ...
      {images.length > 1 && (
        <div className="mt-6 flex justify-center">
          <div className={`grid gap-3 justify-items-center ${
            ...
            images.length === 5 ? 'grid-cols-3 sm:grid-cols-5' :
